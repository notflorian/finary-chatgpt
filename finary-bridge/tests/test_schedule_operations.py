...
def test_repository_workflow_stays_safe_to_import() -> None:
    workflow = json.loads((REPOSITORY_ROOT / "n8n/workflows/finary-mcp-sync.json").read_text())

    assert workflow["active"] is False
    assert workflow["settings"]["timezone"] == "Europe/Paris"
    assert any(node["type"] == "n8n-nodes-base.manualTrigger" for node in workflow["nodes"])

    schedule_nodes = [
        node for node in workflow["nodes"] if node["type"] == "n8n-nodes-base.scheduleTrigger"
    ]
    assert len(schedule_nodes) == 1
    assert schedule_nodes[0]["parameters"]["rule"]["interval"][0]["expression"] == "30 7 * * *"
