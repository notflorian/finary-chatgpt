...
def test_ci_has_stable_read_only_jobs_and_safe_triggers() -> None:
    ...
    assert ci.count("timeout-minutes:") == 8
...
def test_ci_requires_pinned_runtime_execution_after_import():
    ci = CI_PATH.read_text(encoding="utf-8")
    job = ci.split("  n8n-import:", 1)[1].split("  oauth-ownership:", 1)[0]
    assert "bash scripts/validate-n8n-imports.sh" in job
    assert 'FINARY_REQUIRE_N8N_RUNTIME: "1"' in job
    assert 'docker pull "$n8n_image" >/dev/null' in job
    assert "--ci-shard" not in job
    pytest_cmd = (
