...
| #89 | Typed authoritative `/v3/snapshot`; legacy routes preserved | Real SDK → adapter → service/API; every snapshot fixture checked against production models and exported validator | Offline implementation; first live collection failed response validation, structural diagnosis pending |
...
| --- | --- | --- |
| `python -m pytest -m "not live" --ignore=tests/live` from `finary-bridge` | 3,574 passed, 47 skipped, 321.13s | Last full local run, before collection diagnostics; skipped Docker cases are executed separately; live tests were excluded |
...
failed with `MCP_MALFORMED_RESPONSE`; its failing boundary is not yet established.
